...
!      USE CICE_MCT, ONLY: ice_OCNid => OCNid
      ...
      USE ocean_coupler_mod, ONLY : finalize_ocn2cic_coupling
#endif
...
! For CICE-ROMS coupling ??

#ifdef CICE_OCEAN

      DO ic=1,Ncic_grids
        DO io=1,Nocn_grids
          lcm=gcdlcm(dtcic(ic),dtocn(io))
          IF (MOD(TI_CIC2OCN,REAL(lcm,m8)).eq.0) THEN
            nCIC2OCN(ic,io)=INT(TI_CIC2OCN/dtcic(ic))
          ELSE
            lcm=gcdlcm(TI_CIC2OCN,REAL(lcm,m8))
            nCIC2OCN(ic,io)=INT(REAL(lcm,m8)/dtcic(ic))
          END IF
        END DO
      END DO
...
!
      ...
      DO io=1,Nocn_grids
        DO ic=1,Ncic_grids
          lcm=gcdlcm(dtcic(ic),dtocn(io))
          IF (MOD(TI_OCN2CIC,REAL(lcm,m8)).eq.0) THEN
            nOCN2CIC(io,ic)=INT(TI_OCN2CIC/dtocn(io))
          ELSE
            lcm=gcdlcm(TI_OCN2CIC,REAL(lcm,m8))
            nOCN2CIC(io,ic)=INT(REAL(lcm,m8)/dtocn(io))
          END IF
        END DO
      END DO
...
! For CICE-ROMS coupling ??
      ...
      DO ic=1,Ncic_grids
        DO io=1,Nocn_grids
          lcm=gcdlcm(dtcic(ic),dtocn(io))
          IF (MOD(TI_OCN2CIC,REAL(lcm,m8)).eq.0) THEN
            nCICFOCN(ic,io)=INT(TI_CIC2WAV/dtcic(ic))
          ELSE
	    lcm=gcdlcm(TI_OCN2CIC,REAL(lcm,m8))
            nCICFOCN(ic,io)=INT(REAL(lcm,m8)/dtcic(ic))
          END IF
        END DO
      END DO
!
      DO io=1,Nocn_grids
        DO ic=1,Ncic_grids
          lcm=gcdlcm(dtcic(ic),dtocn(io))
          IF (MOD(TI_CIC2OCN,REAL(lcm,m8)).eq.0) THEN
            nOCNFCIC(io,ic)=INT(TI_CIC2OCN/dtocn(io))
          ELSE
            lcm=gcdlcm(TI_CIC2OCN,REAL(lcm,m8))
            nOCNFCIC(io,ic)=INT(REAL(lcm,m8)/dtocn(io))
          END IF
        END DO
      END DO
#endif
...
#ifdef CICE_COUPLING
      peCIC_frst=pelast+1
      peCIC_last=peCIC_frst+NnodesCIC-1
      pelast=peCIC_last
#endif
...
#endif
#ifdef CICE_COUPLING
          WRITE (stdout,23) peCIC_frst, peCIC_last
 23       FORMAT (/,7x,'CICE Model MPI nodes: ',i3.3,' - ', i3.3)
#endif
        END IF
      END IF
!
!  Split the communicator into SWAN, WRF, CICE and ROMS subgroups based 
!  on color and key.
!
      Atmcolor=1
      Ocncolor=2
...
#endif
#ifdef CICE_COUPLING
      IF ((peCIC_frst.le.MyRank).and.(MyRank.le.peCIC_last)) THEN
        MyColor=Ciccolor
      END IF
#endif
